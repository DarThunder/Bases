 ...
 * - Conexión y configuración automática con MongoDB
 * - Gestión completa de usuarios (agregar, mostrar, buscar, eliminar)
 * - Interfaz de consola interactiva para el usuario
 * - Manejo de recursos con cierre adecuado de conexiones
 * 
 * La clase utiliza el patrón Controller del MVC, encapsulando toda la lógica
 * de negocio relacionada con la gestión de usuarios en MongoDB.
 * 
 * @author DarThunder
 * @version 1.0
 */
public class MongoController {
    ...
    /**
     * Constructor que inicializa la conexión a MongoDB y los recursos necesarios.
     * 
     * Establece la conexión utilizando la configuración definida en DatabaseConfig,
     * selecciona la base de datos correspondiente e inicializa el scanner
     ...
     * 
     * Presenta un menú de opciones al usuario y maneja la navegación
     * entre las diferentes operaciones disponibles. El menú se repite
     * hasta que el usuario seleccione la opción de salir.
     * 
     * Opciones disponibles:
     * 1. Agregar usuario - Crea un nuevo usuario en la colección
     * 2. Mostrar todos los usuarios - Lista todos los usuarios existentes
     * 3. Buscar usuario por nombre - Encuentra un usuario específico
     * 4. Eliminar usuario - Borra un usuario de la colección
     * 0. Volver al menú principal - Sale del módulo de gestión
     * 
     * Utiliza un bucle do-while para mantener el menú activo y
     * switch expressions para un manejo limpio de las opciones.
     */
    public void manageUsers() {
        ...
        do {
            ...
            switch (opcion) {
                case 1 -> addUser();                    // Agregar nuevo usuario
                case 2 -> showAllUsers();               // Mostrar todos los usuarios
                case 3 -> findUserByName();             // Buscar usuario específico
                case 4 -> deleteUser();                 // Eliminar usuario
                case 0 -> System.out.println("Volviendo al menú principal..."); // Salir
     ...
     * 
     * Solicita al usuario los datos necesarios (nombre, email, edad)
     * y crea un documento MongoDB con esta información más la fecha
     * de registro automática.
     * 
     * Campos del documento creado:
     * - nombre: String con el nombre del usuario
     * - email: String con el correo electrónico
     * - edad: Integer con la edad
     ...
     * "usuarios" y los muestra utilizando el método printDocument para
     * un formato visualmente atractivo.
     * 
     * Si no hay usuarios en la colección, simplemente no mostrará ningún resultado.
     * La consulta es eficiente y recupera todos los documentos en una sola operación.
     */
     ...
     * 
     * Solicita al usuario el nombre a buscar y realiza una consulta
     * exacta utilizando el filtro eq() de MongoDB. Si encuentra el usuario,
     * lo muestra formateado; si no, informa que no fue encontrado.
     * 
     * La búsqueda es case-sensitive y debe coincidir exactamente con
     * el nombre almacenado en la base de datos.
     */
     ...
     * 
     * Solicita al usuario el nombre del usuario a eliminar y realiza
     * la operación de borrado. Informa el resultado de la operación
     * basándose en el número de documentos eliminados.
     * 
     * La operación elimina únicamente el primer documento que coincida
     * con el nombre especificado (deleteOne). Si existen múltiples usuarios
     * con el mismo nombre, solo se eliminará uno.
     */
     ...
     * 
     * Es importante llamar este método al finalizar el uso del controlador
     * para evitar memory leaks y liberar adecuadamente las conexiones de red.
     * 
